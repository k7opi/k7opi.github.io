...
//解析并获取歌词对象
function getlrc(id){
    let mid = playlist[id].musicrid;
    if(playlist[id].source == 'kuwo'){
        $.ajax({
            type: "GET", 
            url: BASEURL + '/kuwolrc', 
            data: "id="+mid,
            success: function(jsonData){
                ...
                for(let j=0;j<jsonData.length;j++){
                    lrcobj[parseInt(jsonData[j].time)] = jsonData[j].lineLyric;
                }
                rem.lyric = lrcobj;
                lyricCallback(id);
            }   //success
        });//ajax
    }else if(playlist[id].source == 'netease'||playlist[id].source == 'qq'){
        var url = '';
        switch(playlist[id].source){
            case 'netease':
                url = BASEURL + '/neteaselrc';
                break;
            case 'qq':
                url = BASEURL + '/qqlrc';
                break;
        ...
        $.ajax({
            ...
            success: function(jsonData){
                ...
                rem.lyric = parseLyric(jsonData);    // 解析获取到的歌词
                lyricCallback(id);
            }   //success
        });//ajax
    }
...
// 歌曲加载完后的回调函数
// 参数：正在播放的歌曲id序号
function lyricCallback(id) {
    if(id !== rem.playid) return;  // 返回的歌词不是当前这首歌的，跳过
